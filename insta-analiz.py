...
class Credentials:
    def __init__(self):
        if os.environ.get('INSTA_USERNAME') and os.environ.get('INSTA_PASSWORD'):
            ...
            sys.exit('Lütfen Belirtilen Argüman Dizininde Çalıştırın: ./insta-analiz.py USERNAME PASSWORD.\nAnaliz Ediliyor...')
...
def login():
    ...
    if csrf:
        ...
        print("1 Saat Sonra Tekrar Deneyin. Crf Kodu Alınamadı")
        return False
    ...
    if 'two_factor_required' in response_data:
        print('Lütfen 2 Adımlı Doğrulamayı Kapatın.')
        sys.exit(1)

    if 'message' in response_data and response_data['message'] == 'checkpoint_required':
        print('İnstagram Uygulamanızı Açıp Checkpointi Kontrol Edin.')
        sys.exit(1)
...
def unfollow(user):
    ...
    if response['status'] != 'ok':
        print('Tekrar Deneniyor {}. Lütfen Bekleyin...'.format(user['username']))
...
def main():
    ...
    if os.path.isfile(session_cache):
        ...
        is_logged = login()
        if is_logged == False:
            sys.exit('Giriş İşlemi Başarısız')
    ...
    print('Giriş\'Başarılı {} ({} takipçi, {} takip edilen)'.format(connected_user['username'], connected_user['edge_followed_by']['count'], connected_user['edge_follow']['count']))
    ...
    if os.path.isfile(following_cache):
        with open(following_cache, 'r') as f:
            ...
            print('takipçi listesi oluşturuldu')

    if len(following_list) != connected_user['edge_follow']['count']:
        if len(following_list) > 0:
            print('yeniden yapılandırılıyor...', end='', flush=True)
        else:
            print('liste oluşturuluyor...', end='', flush=True)
    ...
    if os.path.isfile(followers_cache):
        with open(followers_cache, 'r') as f:
            ...
            print('takip edilen listesi oluşturuldu')
    ...
    print('Takip edilenler {} kişi. Takip etmeyenler:'.format(len(unfollow_users_list)))
    for user in unfollow_users_list:
        print(user['username'])

    if len(unfollow_users_list) > 0:
        print('Takipten Çıkartılıyor...')

        for user in unfollow_users_list:
            if not os.environ.get('UNFOLLOW_VERIFIED') and user['is_verified'] == True:
                print('Atlanıyor {}...'.format(user['username']))
                continue

            time.sleep(random.randint(2, 4))

            print('Takip Edilmiyor {}...'.format(user['username']))
            while unfollow(user) == False:
                time.sleep(random.randint(1, 3) * 1000) # High number on purpose

        print(' İşlem Tamamlandı - Developed by Burak Eyidoğan')
